	...
	$("input[name=phone]")
		...
		.on("click", function () {
			if ($(this).val() === "+38 (___) ___-____") {
				//перемещаем принудительно курсор на начало ввода номера
...
window.addEventListener("DOMContentLoaded", () => {
	...
	if (
		/Android|webOS|iPhone|iPad|iPod|BlackBerry|BB|PlayBook|IEMobile|Windows Phone|Kindle|Silk|Opera Mini|Safari|Chrome Mobile/i.test(
			user.browser.name
		)
	) {
		//проверка регулярными выражениями на название браузера
	...
	document.body.addEventListener("click", (e) => {
		...
		if (
			...
			e.target.closest(".menu__overlay") ||
